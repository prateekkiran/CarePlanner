...
const dayEventVariants = cva('font-bold border-l-4 rounded p-2 text-xs', {
  variants: {
    variant: {
      default: 'bg-slate-100/60 text-slate-500 border-slate-200',
...
const CalendarWeekView = () => {
  ...
  return (
    <div className="flex flex-col relative overflow-auto h-full">
      <div className="flex sticky top-0 bg-white z-10 border-b mb-3">
        <div className="w-12"></div>
        {headerDays.map((date, i) => (
          <div
            key={date.toString()}
            className={cn(
              'text-center flex-1 gap-1 pb-2 text-sm text-slate-500 flex items-center justify-center',
          ...
          >
            ...
            <span
              className={cn(
                ...
                isToday(date) &&
                  'bg-emerald-500 text-white rounded-full size-6'
              )}
            >
              {format(date, 'd')}
            </span>
        ...
        <div className="grid grid-cols-7 flex-1">
          {weekDates.map((hours, i) => {
            return (
              <div
                className={cn(
                  'h-full text-sm text-slate-500 border-l first:border-l-0',
                  [0, 6].includes(i) && 'bg-slate-100/70'
                )}
...
const CalendarMonthView = () => {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="grid grid-cols-7 gap-px sticky top-0 bg-white border-b">
        {weekDays.map((day, i) => (
          <div
            key={day}
            className={cn(
              'mb-2 text-right text-sm text-slate-500 pr-2',
              [0, 6].includes(i) && 'text-slate-400'
            )}
          >
            {day}
      ...
      <div className="grid overflow-hidden -mt-px flex-1 auto-rows-fr p-px grid-cols-7 gap-px">
        {monthDates.map((_date) => {
          ...
          return (
            <div
              className={cn(
                'ring-1 p-2 text-sm text-slate-500 ring-border overflow-auto',
                !isSameMonth(date, _date) && 'text-slate-400'
              )}
              key={_date.toString()}
            >
              <span
              ...
              {currentEvents.map((event) => {
                return (
                  ...
                  >
                    ...
                    <time className="tabular-nums text-slate-400 text-xs">
            ...
            {days.map((_date) => {
              return (
                <div
                  key={_date.toString()}
                  className={cn(
                    getMonth(_date) !== i && 'text-slate-500'
                  )}
                >
                  <div
                    className={cn(
  ...
  return (
    <div className="pr-2 w-12">
      {Array.from(Array(25).keys()).map((hour) => {
        return (
          <div
            className="text-right relative text-xs text-slate-400 h-20 last:h-0"
            key={hour}
          >
            {now.getHours() === hour && (
              <div
